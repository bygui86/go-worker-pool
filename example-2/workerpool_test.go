...
import (
	"fmt"
	workerpool "github.com/bygui86/go-worker-pool/example-2"
	"sync"
	"testing"
	"time"
)
...
func TestWorkerPool_NewPool(t *testing.T) {
	if _, err := workerpool.NewSimplePool(0, 0); err != workerpool.ErrNoWorkers {
		t.Fatalf("expected error when creating pool with 0 workers, got: %v", err)
	}
	if _, err := workerpool.NewSimplePool(-1, 0); err != workerpool.ErrNoWorkers {
		t.Fatalf("expected error when creating pool with -1 workers, got: %v", err)
	}
	if _, err := workerpool.NewSimplePool(1, -1); err != workerpool.ErrNegativeChannelSize {
		t.Fatalf("expected error when creating pool with -1 channel size, got: %v", err)
	}

	p, err := workerpool.NewSimplePool(5, 0)
	if err != nil {
...
func TestWorkerPool_Work(t *testing.T) {
	...
	p, err := workerpool.NewSimplePool(5, len(tasks))
	if err != nil {
...
func TestWorkerPool_BlockedAddWorkReleaseAfterStop(t *testing.T) {
	p, err := workerpool.NewSimplePool(1, 0)
	if err != nil {
		t.Fatal("error making worker pool:", err)
	}
